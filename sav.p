...
type
   ...
   CandRecord		= record                          {Candidate record}
			     Name	: CandName;
   ...
   RankCriterion	= (Raw, Alloc);    {Which set of votes do we use to
					    rank candidates?}
   VoteType(N:integer)  = array [1..N] of boolean; {true for candidates voted
					 	    for; false otherwise}
   VoteTypePtr	        = ^VoteType;

Procedure AssignVote(	 VoteFor : VoteTypePtr;  {Vote to assign}
		     var CandRec  : CandRecord;   {Record of candidate to whom
						   to assign vote}
...
Procedure TransferVote(var CandRec  : CandRecord;   {Record of candidate from
						     whom to transfer vote}
...
begin
   ...
   for Index:=1 to NCand do
	 ...
	 if RankBy=Raw then
	    RankSetKey(RankList^,Index,@RawVote)
...
begin
   for seq:=1 to nvotes do begin
	 ...
	 if TransTo=0 then TransTo:=Index;       {Highest ranked candidate
						  voted for}
	 if CandList[Index].Status=Elected then continue; {Prefer candidates
							   that have not yet
							   reached quota}
...
begin
   ...
   for Index:=1 to NCand do
      NTrans[Index]:=CandList[Index].Allocation;
   ...
   while (NElect<NSeats) do begin
	 ...
	 break;
      ...
      for Rank:=1 to NCand do begin
	 Index:=RankIndex(CandRank,Rank);
	 if Index=TransFrom then continue; {Skip candidate from whom vote was
					    transferred}
	 if not VoteFor^[Index] then continue; {Didn't vote for this one}
	 if TransTo=0 then Transto:=Index;      {Highest ranked remaining
					 	 candidate voted for}
	 if CandList[index].Status=Elected then continue; {Prefer non-elected
							   candidates}
	 TransTo:=Index; {Pick this one}
	 break;
         end;
...
begin
   for RankFrom:=NCand downto 1 do begin
	 ...
	 for RankTo:=1 to NCand do begin
	    ...
	    if not ((CandList[Index].Status=Neither) and
		    (VoteFor^[Index])) then continue;
	    TransTo:=Index;
	    break;
	    end;
	 if TransTo=0 then TransTo:=TransFrom; {Can't transfer vote}
...
var
   ...
   NWin	  : integer value 0;          {# persons voting for at least one
				       winning candidate}
   VoteFor : VoteTypePtr;             {Points to vote vector}
...
begin
   ...
   writeln;
   writeln("Candidate":MaxCandName,"Raw vote":VoteWidth,
	   "Allocation":VoteWidth,"Status":ElectWidth);
...
begin
   ...
   while not eof(VoteFile) do begin
	 ...
	 VoteFor^[Index]:=false;
      ...
      {Read vote}
      while not eoln(VoteFile) do 
         begin
	 read(VoteFile,Vote);
	 {Ignore any out-of-range votes}
	 if (Vote>0) and (Vote<=NCand) then
            begin
	    VoteFor^[Vote]:=true;
...
{Main Program}
var
   AllocRank : RankArrayPtr value nil; {Ptr to array of candidate indeces
				       in order of allocation}
   CandList  : CandArrayPtr value nil; {Ptr to candidate record list}
   CandRank  : RankArrayPtr value nil; {Ptr to array of candidate indeces in
                                        order of raw vote}
...
begin
   ...
   if NElect<NSeats then
      Alloc2(CandList^,CandRank^,Quota,NSeats,AllocRank^,NElect);
    
   {If not all seats are filled, do third-stage assignment of votes}
   if NElect<NSeats then
      Alloc3(CandList^,CandRank^,NSeats,Quota,AllocRank^,NElect);
